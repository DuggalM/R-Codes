...
qt.first.grid <- qt.master.lst[5]    # inovoke first instance of the QT grid
...
#' first create empty list of centroids
...
for (i in 2:length(qt.master.lst)) {
  
  qt.next.shp <- qt.points[qt.points@data$ID == qt.master.lst[i],]    # select first QT
  ...
  cen <- as.data.frame(cbind(activity.cen.n$cen.x, activity.cen$cen.y, activity.cen$Gid)) %>% 
    group_by(activity.cen.n$cen.x, activity.cen.n$cen.y) %>% 
    summarise(ID=mean(ID))
   
  cen1[i,] <- c(activity.cen.n$cen.x, activity.cen.n$cen.y, activity.cen.n$Gid)
  cen1[i,] <- c(activity.cen.n$cen.x, activity.cen.n$cen.y, activity.cen.n$Gid)
