...
#' User thresholds
du_cnt <- 10000 # Dwelling unit threshold outside the GGH
du_ggh <- 10000 # Dwelling unit threshold within the GGH
...
#' functions. This is used for FRIST set of TAZs only

fun_un1 <- function(field, field1, val, val1, sseq){
  " 
  ...
  Arguments:
  First, dwelling units column in the shapefile table that will be used to subset 
  dataframe.
  Second, truck stops column in the shapefile table that will be used to subset 
  dataframe.
  Third, dwelling unit or DA value by which to subset dataframe i.e. 0 for first case
  and 1 for second case.
  Fourth, TruckStops value by which to subset dataframe i.e. 0 for first case.
  Fifth, starting sequence value.
  ...
  taz <- da1[da1@data[[field]] == val & da1@data[[field1]] == val1 & da1@data[["GGH"]] == 0 & 
               da1@data[["CntDA"]] > 1,]
...
fun_shp <- function(tt, un, taz){
  ...
  tt3 <- merge(tt, un, by.x = "CSDUID.1", by.y = "CSDUID", all.x = TRUE) %>% 
    subset(., select = c("CSDUID", "DisNum", "units", "TruckStops", "dwell"))
  ...
  spdf <- tt3 %>% group_by(DisNum) %>% summarise(DU = sum(dwell), Stops = sum(TruckStops), CSD = min(CSDUID))
...
# field in the shapefile to be used for getting CSDs with no dwelling units. 
# field in the shapefile to be used for getting CSDs with no truck points
...
#' units are lesser than DU threshold, truck stops are below the threshold and 
#' it does not belong in the GGH area
...
#' first deprecate the list of CSDs by removing those that have already been used
...
csd_below <- subset(du_avg_left, (CntDA > 1 & GGH == 0) & ((units > 0 & units <= du_cnt) 
                                                        | (Stops > 0 & Stops <= truck)))
...
euc <- function(i, shp){
  ...
  df <- da_cen_1@data %>% subset(., select = c("DAUID", "CSDUID", "GGH", "dwell", "TruckStops"))
  
  # convert factors to numeric
  indx <- sapply(df, is.factor)
    df[indx] <- lapply(df[indx], function(x) as.numeric(as.character(x)))
    # sort by DAUID
    df1 <- arrange(df, dwell, TruckStops)
...
# Dwelling Units
zone <- function(df1, cc, tf, du_threshold, taz_fourth, truck_threshold){
  ...
  # the cumulative dwellings and truck stops for the DAs in ascending distance.
  tf_1 <- merge(cc_f, tf, by.x = "Dest", by.y = "DAUID", all.x = TRUE) %>% 
    arrange(., Dist) %>% 
    transform(., CumDU = cumsum(dwell)) %>% transform(., CumTS = cumsum(TruckStops))
  ...
  tf_1_dis <- subset(tf_1, CumDU <= du_threshold | CumTS <= truck_threshold) 
  ...
  # prepare table to attach to dissolved polygons to make into spatial dataframe
  spdf <- tf_1_dis %>% group_by(DisNum) %>% summarise(DU = sum(dwell), TS = sum(TruckStops))
    spdf <- as.data.frame(spdf)
    rownames(spdf) <- spdf$DisNum
  ...
  # Return
  list(tf_1_dis, df_leftover, cc_left, taz4_spdf, da_sh, tf_1)
...
for (i in 1:1){
  ...
  f_da <- zone(df1, cc, tf, du_cnt, taz_fourth, truck)
  ...
  # try3 <- f_da[[1]]
  # 
  # a1 <- subset(try2, CumDU <= 2500 & CumTS <= 5000) 
  
  while (nrow(df_leftover)> 1){
    
      # Start with the second DA that is leftover
      s_da <- zone(df_leftover, cc_f, tf, du_cnt, taz_fourth, truck)
...
if (nrow(df_leftover) == 1) {
  ...
  taz4_single <- da_poly[da_poly@data$DAUID == df_leftover$DAUID[1], ]
    taz4_single <- taz4_single[, c("DAUID", "dwell", "TruckStops")]
...
if (nrow(df_leftover) == 1) {
  ...
  taz4_single1 <- da_poly[da_poly@data$DAUID == df_leftover$DAUID[1], ]
    taz4_single1 <- taz4_single1[, c("DAUID", "dwell", "TruckStops")]
      taz4_single1@data <- rename(taz4_single1@data, DisNum = DAUID, DU = dwell, TS = TruckStops)
        rownames(taz4_single1@data) <- taz4_single1@data$DisNum

    # reset polygon ID of DA
    taz4_single1@polygons[[1]]@ID <- as.character(taz4_single1@data$DisNum)
...
#' Get all the CSDs that have only one TAZ. The TAZ is a provincial zone in this case
ggh_single <- tf %>% group_by(CSDUID) %>% summarise(CntTAZ = n()) %>% subset(., CntTAZ == 1)

# CSDs below the threshold and with only 1 TAZ in it
csd_ggh_b <- subset(csd_ggh, units <= du_ggh & Stops < truck | csd_ggh$CSDUID %in% ggh_single$CSDUID)
...
# prepare table to attach to dissolved polygons to make into spatial dataframe
spdf <- ft %>% group_by(CSDUID) %>% summarise(DU = sum(dwell), TS = sum(TruckStops))
  spdf <- as.data.frame(spdf)
...
taz5_spdf <- SpatialPolygonsDataFrame(taz5, spdf) 
  colnames(taz5_spdf@data) <- c("DisNum", "DU", "TS")
...
# each other for one consolidated set of zones under the FIFTH CASE.
  ...
  for (i in 1:1){
    ...
    # Start with the first DA
    f_da <- zone(df1, cc, tf, du_ggh, taz_fifth, truck)
    # get each element of the output from the zone list
    taz5_prov <- f_da[[4]]
    df_leftover <- f_da[[2]]
    cc_f <- f_da[[3]]
    ...
    while (nrow(df_leftover)> 1){
      ...
      # Start with the second DA that is leftover
      s_da <- zone(df_leftover, cc_f, tf, du_ggh, taz_fifth, truck)
    ...
    if (nrow(df_leftover) == 1) {
      ...
      taz5_single <- taz5_single[, c("DAUID", "dwell", "TruckStops")]
      taz5_single@data <- rename(taz5_single@data, DisNum = DAUID, DU = dwell, TS = TruckStops)
...
# Now run the dissolving procedures for the leftover TAZs 
  ...
  for (i in 2:nrow(lst)){
    ...
    if (nrow(df_leftover) == 1) {
      ...
      taz5_single1 <- taz5_single1[, c("DAUID", "dwell", "TruckStops")]
      taz5_single1@data <- rename(taz5_single1@data, DisNum = DAUID, DU = dwell, TS = TruckStops)
...
all_taz <- spRbind(all_taz, third_taz) %>% 
  subset(., select = c("DisNum", "DU", "Stops")) 
  colnames(all_taz@data) <- c("DisNum", "DU", "TS")
...
writeOGR(first_taz, layer = paste0("FirstTAZ_GA", du_cnt, truck), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(sec_taz, layer = paste0("SecondTAZ_GA", du_cnt, truck), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(third_taz, layer = paste0("ThirdTAZ_GA", du_cnt, truck), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(taz4_prov, layer = paste0("Fourth_GA",du_cnt, truck), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(taz5_prov, layer = paste0("Fifth_GA", du_cnt, truck), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(all_taz, layer = paste0("All_GA_Zones", " OR ", du_cnt, truck), wd, 
