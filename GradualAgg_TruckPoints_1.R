...
#' User thresholds
ap_cnt <- 3000 # Activity threshold outside the GGH
ap_ggh <- 3000 # Activity threshold within the GGH
...
csd_below <- subset(du_avg_left, (CntDA > 1 & GGH == 0) & (TotAct > 0 & TotAct <= ap_cnt)) 
...
tf <- taz_fourth@data %>% subset(., select = c(DAUID, CSDUID, dwell, TruckStops, TotAct, CSD_TotAct)) 
...
euc <- function(i, shp){
  ...
  df <- da_cen_1@data %>% subset(., select = c("DAUID", "CSDUID", "GGH", "dwell", "TruckStops", "TotAct"))
  
  # convert factors to numeric
  indx <- sapply(df, is.factor)
    df[indx] <- lapply(df[indx], function(x) as.numeric(as.character(x)))
    # sort by DAUID
    df1 <- arrange(df, desc(TotAct))
...
zone <- function(df1, cc, tf, du_threshold, taz_fourth, truck_threshold){
  ...
  # the cumulative activity points for the DAs in ascending distance.
  ...
  tf_1_dis <- subset(tf_1, CumAct <= ap_cnt) 
  ...
  # prepare table to attach to dissolved polygons to make into spatial dataframe
  spdf <- tf_1_dis %>% group_by(DisNum) %>% summarise(ZoneAct = sum(TotAct))
    spdf <- as.data.frame(spdf)
    rownames(spdf) <- spdf$DisNum
...
if (nrow(df_leftover) == 1) {
  ...
  taz4_single <- da1[da1@data$DAUID == df_leftover$DAUID[1], ]
    taz4_single@data <- transform(taz4_single@data, ZoneAct = TotAct)
      taz4_single@data <- rename(taz4_single@data, DisNum = DAUID)
      #' keep only two columns
      taz4_single <- taz4_single[, c("DisNum", "ZoneAct")]
...
if (nrow(df_leftover) == 1) {
  ...
  taz4_single1 <- da1[da1@data$DAUID == df_leftover$DAUID[1], ]
    taz4_single1@data <- transform(taz4_single1@data, ZoneAct = TotAct)
      taz4_single1@data <- rename(taz4_single1@data, DisNum = DAUID)
      #' keep only two columns
      taz4_single1 <- taz4_single1[, c("DisNum", "ZoneAct")]
...
csd_ggh_b <- subset(csd_ggh, TotAct <= ap_ggh | csd_ggh$CSDUID %in% ggh_single$CSDUID)
...
# each other for one consolidated set of zones under the FIFTH CASE.
  ...
  for (i in 1:1){
    ...
    while (nrow(df_leftover)> 1){
      ...
      # Start with the second DA that is leftover
      s_da <- zone(test, cc_f, tf, du_ggh, taz_fifth, truck)
    ...
    if (nrow(df_leftover) == 1) {
      ...
      taz5_single <- ggh[ggh@data$DAUID == df_leftover$DAUID[1], ]
        taz5_single@data <- transform(taz5_single@data, ZoneAct = TotAct)  
        taz5_single@data <- rename(taz5_single@data, DisNum = DAUID)
        #' Keep only two columns
        taz5_single <- taz5_single[, c("DisNum", "ZoneAct")]
  ...
  for (i in 2:nrow(lst)){
    ...
    if (nrow(df_leftover) == 1) {
      ...
      taz5_single1 <- ggh[ggh@data$DAUID == df_leftover$DAUID[1], ]
        taz5_single1@data <- transform(taz5_single1@data, ZoneAct = TotAct)
          taz5_single1@data <- rename(taz5_single1@data, DisNum = DAUID)
          #' keep only two names
          taz5_single1 <- taz5_single1[, c("DisNum", "ZoneAct")]
...
all_taz <- spRbind(all_taz, third_taz) %>% 
  subset(., select = c("DisNum", "Act")) 
  colnames(all_taz@data) <- c("DisNum", "ZoneAct")
...
writeOGR(first_taz, layer = paste0("FirstTAZ_GA", ap_cnt), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(sec_taz, layer = paste0("SecondTAZ_GA", ap_cnt), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(third_taz, layer = paste0("ThirdTAZ_GA", ap_cnt), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(taz4_prov, layer = paste0("Fourth_GA",ap_cnt), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(taz5_prov, layer = paste0("Fifth_GA", ap_ggh), wd, driver="ESRI Shapefile", overwrite_layer = T)
writeOGR(all_taz, layer = paste0("All_GA_Zones", ap_cnt, "_", ap_ggh), wd, 
