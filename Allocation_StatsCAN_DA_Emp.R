...
gghm.poly <- readOGR(wd, gghm.pol)
  ...
  da.poly <- da.poly[, c("CSDUID", "DAUID", "GGH", "dwell", "TotalEmp")]
  da.cen <- da.cen[, c("CSDUID", "DAUID", "GGH", "dwell", "X", "Y", "TotalEmp")]
...
pix.csd.da <- point.in.poly(nl.20.sub, da.poly)
  ...
  pix.csd.da@data <- transform(pix.csd.da@data, Frac = PixVal / TotPixval) %>%
    transform(., PixEmp = Frac * TotalEmp) 
  
  print(paste0("The total jobs assigned to the pixel points are ", 
               sum(pix.csd.da@data$PixEmp)))
...
da.cen.rm <- da.cen
  ...
  da.cen.rm <- da.cen.rm[, c("CSDUID", "DAUID", "GGH", "X", "Y", "dwell", "remove", "TotalEmp")]
...
pix.csd.da <- pix.csd.da[, c("CSDUID", "x", "y", "GGH", "PixEmp")]

  #' rename columns and add in DAUID
  names(pix.csd.da@data) <- c("CSDUID", "X", "Y", "GGH", "TotalEmp")
  pix.csd.da@data <- transform(pix.csd.da@data, DAUID = 0)

#' Only keep the columns needed for the DA centroids
da.cen.rm <- da.cen.rm[, c("CSDUID", "X", "Y", "GGH", "TotalEmp", "DAUID")]
...
activity_centroids_emp <- rbind(pix.csd.da, da.cen.rm)
  sum(activity_centroids_emp@data$TotalEmp)

 ac <- activity_centroids_emp@data
###############################################################################
#' Step 6: Clean up Activity Centroids file for input in to the Centroid Script
###############################################################################
  
#' rename columns and add in DAUID
names(activity_centroids_emp@data) <- c("CSDUID", "Xcoord", "Ycoord", "GGH", "val", "DAUID")

#' write out shapefile for input into the Centroid Script
writeOGR(activity_centroids_emp, layer = paste0("ActivityCentroidsEmp"), wd, 
         drive = "ESRI Shapefile", overwrite_layer=T)
