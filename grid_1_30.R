...
for (i in 1:length(G2.lst)) {
  ...
  minx = minx +15000    # to dampen any offsetting in the new grid
  miny = miny +15000    # to dampen any offsetting in the new grid
...
for (i in 1:length(G3.above.thold.lst)) {
  ...
  minx = minx +7500    # to dampen any offsetting in the new grid
  miny = miny +7500    # to dampen any offsetting in the new grid
...
for (i in 1:length(G4.above.thold.lst)) {
  ...
  minx = minx +3750    # to dampen any offsetting in the new grid
  miny = miny +3750    # to dampen any offsetting in the new grid
...
for (i in 1:length(G5.above.thold.lst)) {
  ...
  minx = minx +(3750/2)    # to dampen any offsetting in the new grid
  miny = miny +(3750/2)    # to dampen any offsetting in the new grid
...
for (i in 1:length(G6.above.thold.lst)) {
  ...
  minx = minx +(3750/4)    # to dampen any offsetting in the new grid
  miny = miny +(3750/4)    # to dampen any offsetting in the new grid
...
#' select those grids from G6.comb that are yet above the threshold
#' even after splitting. These will be further cut into half

#+ this is the master dataset of points inside the split grids from G2.comb
sp.join.G6.comb <- over(G6.comb, da[19:20], fn=sum) 
sp.join.G6.comb$gridID <- rownames(sp.join.G6.comb) 

G7 <- G6.comb
G7@data <- merge(G7@data,sp.join.G6.comb, by.x="ID", by.y="gridID", all.x=TRUE)
# remove NA values because they represent grids that must not be split and export
G7.nopnts1 <- G7[is.na(G7@data$dwell),]
writeOGR(G7.nopnts1, layer = "G7_noPoints", wd, drive = "ESRI Shapefile", overwrite_layer=T)
# remove grids that are below the user defined threshold (THOLD)
# remove NA values and export shapefile
G7.noNA <- sp.na.omit(G7)
G7.below.thold <- G7.noNA[G7.noNA@data$dwell < thold,]
writeOGR(G7.below.thold, layer = "G7_below", wd, drive = "ESRI Shapefile", overwrite_layer=T)

#' select those grids that were above the threhold (THOLD) 
#' and split these further
G7.above.thold <- G7.noNA[G7.noNA@data$dwell > thold,]
writeOGR(G7.above.thold, layer = "G7_above", wd, drive = "ESRI Shapefile", overwrite_layer=T)


# select each individual polygon in G3_above and 
# split into four parts

G7.above.thold.lst <- as.list(G7.above.thold@data$ID)    # use list to enumerate grid cells

for (i in 1:length(G7.above.thold.lst)) {
  R6 <- G7.above.thold[G7.above.thold$ID==G7.above.thold.lst[i],]
  ...
  minx = minx +(3750/8)    # to dampen any offsetting in the new grid
  miny = miny +(3750/8)    # to dampen any offsetting in the new grid
  
  #+ create grid
  G7.above.split=GridTopology(c(minx, miny), c(l,l), c(Nbx, Nby))
  G7.above.split=SpatialGrid(G7.above.split)
  G7.above.split=as(G7.above.split, 'SpatialPolygons')
  
  # extract polygon Id's to make dataframe
  Gid.split <- sapply(slot(G7.above.split, "polygons"), function(x) slot(x, "ID")) 
  # Create dataframe with correct rownames
  G7.above.split.df <- data.frame( ID=1:length(G7.above.split), row.names = Gid.split)  
  G7.above.split <- SpatialPolygonsDataFrame(G7.above.split, data=G7.above.split.df)   # make Polygondataframe to export as shapefile
  proj4string(G7.above.split) <- CRS(proj4string(prov))
  
  writeOGR(G7.above.split, layer = paste0('R6_', G7.above.thold.lst[i]), "c:/personal/r/G7Split", driver="ESRI Shapefile", overwrite_layer=T)
...
wd6 <- setwd("c:/personal/r/G7split")

files <- list.files(pattern = "R6.*.shp")
...
# ain't that spatial
spatialPolygons <- SpatialPolygons(polygons)
G7.comb <- SpatialPolygonsDataFrame(spatialPolygons, 
                                    data.frame(ID=1:length(polygons)))
proj4string(G7.comb) <- CRS(proj4string(G2))

writeOGR(G7.comb, layer = paste0("G7_split_combined"), "c:/personal/r/G7Split", driver="ESRI Shapefile", overwrite_layer=T )
