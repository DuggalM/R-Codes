...
obs_hvy <- read.csv("C:/Projects/Province-wide/Empties/empty_heavy_truck_matrix_all.csv", 
                stringsAsFactors = FALSE)

obs_med <- read.csv("C:/Projects/Province-wide/Empties/empty_straight_truck_matrix_all.csv", 
                    stringsAsFactors = FALSE)

#' Combine all empties and also cast it in a format desired and getting rid of 
#' intraCSD trips.
all_obs <- rbind(obs_hvy, obs_med) %>% group_by(Orig, Dest) %>% 
  summarise(TTrips = sum(Trips)) %>% 
  transform(., Flag = paste0(Orig,Dest)) %>% 
  subset(., Orig != Dest)
#' wide format and set NAs to zero
all_obs_cast <- dcast(all_obs, Orig ~ Dest, value.var = "TTrips")
rownames(all_obs_cast) <- all_obs_cast[, 1]
all_obs_cast <- all_obs_cast[, 2:294]
