...
for (j in 2:nrow(lst)){
  ...
  while (nrow(tdf) > 1){
    ...
    while (round4_spdf@data$CumAct < ap_ggh && nrow(tdf) > 0 && nrow(rel_df) > 0){
...
ggh_zones <- round1_spdf
writeOGR(ggh_zones, layer = paste0("TRESO5_1", ap_ggh), wd, 
         driver="ESRI Shapefile", overwrite_layer = T)
...
#rm(round1_spdf, round2_spdf, round3_spdf, round4_spdf)
...
#' remove the CSD # 3549005 as this is a multipart CSD and cause for significant
...
for (j in 2:nrow(lst)){
  ...
  while (round3_spdf@data$CumAct < ap_cnt && nrow(t@data)>0 && 
         nrow(tdf) > 0 && nrow(rel_df > 0)){
